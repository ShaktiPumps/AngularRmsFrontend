...
export class Simha2Component implements OnInit, OnDestroy {
  ...
  onStartDateChange() {
    ...
    this.basicForm.get('endDate')?.setValue(''); // Reset End Date when Start Date changes
  }
  ...
  onSearch() {
    ...
    const requestData = {
      ...
      this.tablesService.getData(requestData); // No need for subscribe here
      
    // this.tablesService.getData(requestData).subscribe({
    //   next: (response) => {
    ...
    //   }
    // });
    // console.log('requestData ' + requestData.startDate);
