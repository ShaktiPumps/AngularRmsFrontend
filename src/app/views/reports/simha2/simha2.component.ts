...
export class Simha2Component implements OnInit, OnDestroy {
  ...
  onStartDateChange() {
    const startDate = this.basicForm.get('startDate')?.value;
    this.minEndDate = startDate ? new Date(startDate) : null;
    this.basicForm.get('endDate')?.setValue(''); 
  }
  ...
  onSearch() {
    ...
    const requestData = {
      ...
      this.tablesService.fetchCumulativeData(requestData);

      this.http.post('http://localhost:9880/RMS/Report/simhaCumu', requestData)
      .subscribe(
        (response: any) => {
          console.log("Cumulative Data:", response);
          this.cumulativeData = response;
          this.dataService.setCumulativeData(this.cumulativeData);
        },
        (error) => {
